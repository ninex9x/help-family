...
export async function openPool(config) {
  ...
  try {
    const result = await pool.query(`SELECT r.rolsuper, r.rolbypassrls,
      EXISTS (SELECT 1 FROM pg_class c WHERE c.relname IN ('families','family_memberships','members','medicines','medicine_presentations','routines','dose_logs') AND c.relowner=r.oid) AS owns_tables
