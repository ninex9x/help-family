...
function documentPage(title, content, className = '') {
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="color-scheme" content="light dark"><title>${e(title)} — help-family</title>${['base', 'layout', 'components', 'family', 'forms', 'accounts', 'members', 'medicines', 'catalog', 'routines'].map((name) => `<link rel="stylesheet" href="/assets/${name}.css">`).join('')}</head><body class="accounts-body ${className}">${content}</body></html>`;
}
